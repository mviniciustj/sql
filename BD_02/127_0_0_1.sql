...
DROP DATABASE IF EXISTS `mvtecnologies`;
CREATE DATABASE IF NOT EXISTS `mvtecnologies` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `mvtecnologies`;

DROP TABLE IF EXISTS `clientes`;
CREATE TABLE IF NOT EXISTS `clientes` (
  `IdCliente` tinyint(4) NOT NULL AUTO_INCREMENT,
  ...
  UNIQUE KEY `NFantasia` (`NFantasia`),
  UNIQUE KEY `Responsavel` (`Responsavel`),
  UNIQUE KEY `Responsavel_2` (`Responsavel`)
) ENGINE=MyISAM AUTO_INCREMENT=4 DEFAULT CHARSET=utf8;

INSERT INTO `clientes` (`IdCliente`, `NFantasia`, `Responsavel`, `Email`, `Telefone`, `Endereco`, `Bairro`, `Cidade`) VALUES
(1, 'Suporponto e Acesso', 'Marcos Santos', 'marcos@suporponto.com.br', '712222-3333', 'R. Cosme de Farias 5a', 'Boca do Rio', 'Salvador'),
(2, 'Coil Bobinas Termicas', 'Verônica Santos', 'veronica@coilcomercio.com.br', '7122223334', 'R. Cosme de Farias 5a', 'Boca do Rio', 'Salvador'),
(3, 'Iwwa Agência Web', 'Fagner Guimarães', 'contato@iwwa.com.br', '712222-3335', 'Rua Irará 22', 'Rio Vermelho', 'Salvador');
DROP DATABASE IF EXISTS `novaondina`;
CREATE DATABASE IF NOT EXISTS `novaondina` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `novaondina`;
...
CREATE TABLE IF NOT EXISTS `publicacoes` (
  ...
  `Nome` varchar(30) NOT NULL,
  `Tipo` enum('Livro','Revista','Folheto','Brochura','Bíblia') DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `publicadores` (
  ...
  `Estado_Naturalidade` varchar(30) DEFAULT NULL,
  `Nascimento` date DEFAULT NULL,
  `Batismo` date DEFAULT NULL,
  `Situacao_Espiritual` enum('Publicador','Pioneiro','Servo Ministerial','Ancião') DEFAULT NULL,
  `Ungido` enum('Sim','Nao') DEFAULT NULL,
  PRIMARY KEY (`Id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
COMMIT;
